  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className={`relative h-full rounded-[2.5rem] p-[1.5px] transition-all duration-500 overflow-hidden bg-slate-200 ${
          isHovered
            ? "bg-gradient-to-tr from-emerald-500 to-blue-500 shadow-md"
            : "shadow-sm"
        }`}
      >
        <article className="relative flex h-full flex-col overflow-hidden rounded-[2.4rem] bg-white transition-all duration-300">
          {/* Glass shine element */}
          <div
            className={`absolute inset-0 w-[200%] h-full bg-gradient-to-r from-transparent via-white/20 to-transparent -skew-x-25 -translate-x-[150%] transition-transform duration-1000 ease-out pointer-events-none z-30 ${
              isHovered ? "translate-x-[120%]" : ""
            }`}
          />

          {/* Image Container */}
          <div className="relative overflow-hidden aspect-[16/10] rounded-t-[2.4rem] w-full z-10">
            <SkeletonImage
              src={blog.image}
              alt={blog.title}
            ...
            {/* Category Badge */}
            <span className="absolute left-6 top-6 rounded-full bg-emerald-600 px-4 py-1 text-[10px] font-bold text-white uppercase tracking-wider shadow-md z-20">
              {blog.category}
            </span>
          </div>
          ...
          <div className="flex flex-col justify-between flex-grow p-6 sm:p-7 space-y-5 z-20">
            <div className="space-y-3">
              <h2 className="text-xl sm:text-2xl font-bold leading-tight text-slate-900 transition-colors duration-300 group-hover:text-emerald-600 line-clamp-2">
                {blog.title}
              </h2>

              <p className="line-clamp-3 text-xs sm:text-sm leading-relaxed text-slate-600 font-light">
                {blog.description}
              </p>
            </div>

            {/* Metadata Footer */}
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-slate-500 font-medium border-t border-slate-100 pt-4">
              <div className="flex items-center gap-2">
                <div className="flex h-6 w-6 items-center justify-center rounded-full bg-emerald-500/10 text-emerald-600 font-bold border border-emerald-500/20 text-[9px]">
              ...
              <div className="flex items-center gap-1.5">
                <FaClock size={10} className="text-emerald-600/70" />
              ...
              <div className="flex items-center gap-1.5">
                <FaEye size={10} className="text-emerald-600/70" />
                <span>{fakeViews}</span>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-1">
              <div className="relative overflow-hidden rounded-full bg-emerald-600 hover:bg-emerald-700 px-5 py-2.5 text-xs font-bold text-white transition-all duration-300 shadow-md hover:shadow-lg flex items-center gap-1.5 cursor-pointer">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={toggleLike}
                  className="transition duration-300 hover:scale-110 p-2 rounded-full bg-slate-100 hover:bg-red-500/10 text-slate-400 hover:text-red-500 cursor-pointer"
                >
                  {liked ? (
                ...
                <button
                  onClick={toggleSave}
                  className="transition duration-300 hover:scale-110 p-2 rounded-full bg-slate-100 hover:bg-emerald-500/10 text-slate-400 hover:text-emerald-500 cursor-pointer"
                >
                  {saved ? (
                    <FaBookmark className="text-xs text-emerald-600" />
                  ) : (
